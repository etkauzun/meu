function [approx, error, log_err, log_n] = corr_trap_appr(a,b,n,str_func)
% corr_trap_appr computes the corrected trapezoid approximation for a given function
%
% Call format: [approx,error,log_err,log_n] = corr_trap_appr(a,b,n,str_func)
%
% input variables:
...
approx = approx + h * ( (-1/24) * ( 3*f(a) - 4*f(a+h) + f(a+(2*h)) ...
    + f(b-(2*h)) - 4*f(b-h) + 3*f(b)) ); % calculating the correction in approximation
